...
def tweet_location(tweet):
    ...
    position = make_position(tweet['latitude'], tweet['longitude'])
    return position
...
def extract_words(text):
    ...
    for letter in text:
        ...
        if (letter not in ascii_letters) and word != '':
            words.append(word)
            word = ''
...
def analyze_tweet_sentiment(tweet):
    ...
    for word in words:
        ...
        word_s = get_word_sentiment(word)


        if has_sentiment(word_s) == True:

            value = sentiment_value(word_s)
            total += value
            counter += 1
            value_exists = True
...
def find_centroid(polygon):
    ...
    while (i < len(polygon)-1):
        ...
        Cx = Cx + (latitude(polygon[i])+ latitude(polygon[i+1]))*(latitude(polygon[i])*longitude(polygon[i+1]) - latitude(polygon[i+1])*longitude(polygon[i]))
        Cy = Cy + (longitude(polygon[i]) + longitude(polygon[i+1]))*(latitude(polygon[i])*longitude(polygon[i+1]) - latitude(polygon[i+1])*longitude(polygon[i]))

        A = A + 0.5*(latitude(polygon[i])*longitude(polygon[i+1]) - latitude(polygon[i+1])*longitude(polygon[i]))
...
def find_state_center(polygons):
    ...
    while (i < len(polygons)):
        scx += find_centroid(polygons[i])[0]*find_centroid(polygons[i])[2]
        scy += find_centroid(polygons[i])[1]*find_centroid(polygons[i])[2]
        sumareas += find_centroid(polygons[i])[2]
        i += 1

    scx /= sumareas
    scy /= sumareas

    return make_position(scx, scy)
...
def group_tweets_by_state(tweets):
    ...
    def distance(loc1, loc2):
        ...
        d = sqrt((latitude(loc1) - latitude(loc2))**2 + (longitude(loc1) - longitude(loc2))**2)

        return d

    for tweet in tweets:
        closest = 'CA' #default value
        for state in us_states.keys():
            if distance(tweet_location(tweet), find_state_center(us_states[state])) < distance(tweet_location(tweet), find_state_center(us_states[closest])):
                closest = state
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state.keys():
        ...
        for tweet in tweets_by_state[state]:
            ...
            if analyze_tweet_sentiment(tweet) != make_sentiment(None):
            
                if type(sentiment_value(analyze_tweet_sentiment(tweet))) == float:
